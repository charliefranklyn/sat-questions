...
const mathSection = curriculum.find((s) => s.slug === "math")!;
  ...
  return (
    <div className="px-10 py-8 max-w-3xl">
      <nav className="flex items-center gap-1.5 text-sm text-slate-400 mb-6">
        ...
        <span>/</span>
        <span className="text-slate-700 font-medium">Math</span>
      </nav>

      <h1 className="text-3xl font-bold text-slate-900 mb-1">Math</h1>
      <p className="text-slate-400 text-sm mb-10">
        {mathSection.domains.length} domains · {mathSection.domains.reduce((n, d) => n + d.subtopics.length, 0)} topics
      </p>

      <div className="space-y-4">
        {mathSection.domains.map((domain) => (
          <div id={domain.slug} key={domain.slug} className="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div>
              {domain.subtopics.map((sub, i) => (
                <Link
                  key={sub.slug}
                  href={`/math/${domain.slug}/${sub.slug}`}
